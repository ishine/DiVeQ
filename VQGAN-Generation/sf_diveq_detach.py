...
class SFDIVEQDetach(nn.Module):
    """
    SF-DiVeQ_Detach: Detach version of Space-Filling Differentiable Vector
    Quantization (VQ) module that allows end-to-end training of VQ-based models without
    any auxiliary losses or hyperparameter tunings. The module does not need codebook
    replacement, because its training strategy inherently pulls all codewords inside
    the latent space. The module uses a custom initialization such that it skips
    quantization for the first `skip_iters` iterations, and then it initializes the
    codebook by the recent latent vectors and starts quantizing the latents.
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, float]:
        ...
        if self.iter_counter.item() >= self.skip_iters:
            ...
            error_magnitude2 = torch.linalg.norm(codebook_second - z, dim=1,
                                                                        keepdim=True)
            vq_error1 = error_magnitude1 * (((1 - interp_lambda) * (codebook_first - z))
                                            / (error_magnitude1 + 1e-12)).detach()
            vq_error2 = error_magnitude2 * ((interp_lambda * (codebook_first - z))
                                            / (error_magnitude2 + 1e-12)).detach()
    ...
    def inference(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, float]:
        ...
        # used only for training the transformer
        indices_for_transformer = indices_clamped + offset

        return z_q_hard, indices, perplexity, indices_for_transformer
    ...
    def _check_input(self, z: torch.Tensor) -> None:
        ...
        if z.size(1) != self.embedding_dim:
            raise ValueError(f"SF-DiVeQ's input.shape[1] must match the embedding"
    ...
    # Sampling from SF-DiVeQ curve when training the transformer
    def _sample_dithered_codebook(self, indices: torch.Tensor)-> torch.Tensor:
        dither = torch.rand((indices.shape[0], 1), device=indices.device)
        c1 = self.codebook[indices]
        c2 = self.codebook[indices + 1]
        sampled_codebook = ((1 - dither) * c1) + (dither * c2)
